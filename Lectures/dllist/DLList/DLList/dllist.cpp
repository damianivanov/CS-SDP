...
{
    first.next = new DLList<T>::box{ x,first.next,&first };
   /* if (other.first.n == nullptr)
        first = nullptr;
    else
    {
        first = new DLList<T>::box{other.first.data,other.first.next,other.first.prev};
        DLList<T>::box* curr = other.first.next;
        DLList<T>::box* temp = first;
        while (curr != nullptr)
        {
            temp->next = new  DLList<T>::box{};
            temp = temp->next;
            curr = curr->next;
        }
    }*/
